...
public abstract class IUserService : IService<User, IUserRepository, UserSearchParameter>
{
    ...
    // User self-update
    public abstract Task<User> UpdateProfileAsync(string userId, string? name, string? email, string? document, DateOnly? birthDate, List<string>? phones, string? password, CancellationToken cancellationToken = default);
